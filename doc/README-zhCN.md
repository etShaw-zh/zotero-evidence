...
- 选定一个 Codebook 变量，以表格形式查看该变量在全项目范围内所有已确认的值/原文引用，每行标注来源文献。
- 一键运行 AI 主题分析：将各行归并为若干高层主题（同一主题的行使用完全相同的标签）并保存结果；再次运行会整表重新生成。
...
- **导出主题分析数据**：全项目范围内所有已确认的变量/值/原文引用及其主题。
...
6. 依次处理 `TA-Screen Queue` → `FT-Screen Queue` → `Extract Coding`：标题摘要阶段和库视图摘要在条目侧边栏完成，全文阶段和编码在 PDF 阅读器侧边栏完成。
7. 证据编码到一定程度后，使用 **文件 → 主题综合分析…** 查看某个 Codebook 变量在各研究间的主题分布。
8. 撰写综述报告时，使用 **导出 PRISMA 数据…** / **导出筛选决策日志…** / **导出编码数据…** / **导出主题分析数据…**。
